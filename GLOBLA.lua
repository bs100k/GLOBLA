...
local devzain_ = redis:get(GLOBLA.."Bot:KpOwnerBot")
local text = "List of Authorities :\n"
local user_info_ = redis:get(GLOBLA.."user:Name" .. devzain_)
...
local devzain = redis:get(GLOBLA.."Bot:KpOwnerBot")
...
-----------------------function is_devzain----------by Globla------------------------
local is_devzain = function(msg)
local var = false
if msg.sender_user_id_ == tonumber(devzain) then
var = true
end
return var
end
...
local var = false
if user_id == tonumber(devzain) then
var = true
end
return var
end
...
local channel = ''..redis:get(GLOBLA..'Globla')..''
...
if run == "False" or bot_id == 0 or devzain == 0 then
...
redis:set(GLOBLA.."KpOwnerBot" .. devzain, "> Bot Owner ID : " .. botowneruserid .. [[
...
getUser(devzain, outputbotowner)
--------------------function outputbot----------------------------------------
...
redis:set(GLOBLA..'Globla',CH_BOT)
...
redis:set('GLOBLA:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
redis:set('GLOBLA:'..bot_id..'id:user'..msg.chat_id_,taha)  
...
redis:set('GLOBLA:'..bot_id..'idgem:user'..msg.chat_id_,taha)  
...
if not redis:get(GLOBLA..'Globla') then
send(msg.chat_id_, msg.id_, 1, '™️┇ لم يتم وضع القناة\n🔘┇ لتعين  القناة ارسل .....\n☑┇ `تعين قناة الاشتراك `\n➖', 1, 'md')
return false 
end
if redis:get(GLOBLA..'Globla') then
...
if text == '/start' then 
if tonumber(msg.sender_user_id_) == tonumber(devzain) then
...
if text:match("^المجموعات 📇$") and tonumber(msg.sender_user_id_) == tonumber(devzain) then
...
local Globla = redis:get(GLOBLA.."Globla")
if Globla then
send(msg.chat_id_, msg.id_, 1, '📡┇ *قناة الاشتراك* : ['..Globla..']', 1, 'md')
else
...
if tonumber(result.id_) == tonumber(devzain) then
t = "مطور اساسي 👨‍🔧"
...
if result.id_ then
if tonumber(result.sender_user_id_) == tonumber(devzain) then
