...
io.write("\27[0;33m>>"..[[
Send Your iD Sudo : ]].."\n\027[00m")
local TgOwner_ = tonumber(io.read())
if not tostring(TgOwner_):match('%d+') then
TgOwner_ = 352568466
end
io.write("\27[0;36m>>"..[[
Send ( Token )Bot : ]].."\n\027[00m")
...
GLOBLA_SUDO = sudos.TgOwner   -------bot owner
...
local bot_id = redis:get(GLOBLA.."Bot:TgBotAccount") or tonumber(_redis.Bot_ID)
...
redis:sadd(GLOBLA.."Bot:TgSudos", user)
end
redis:setex(GLOBLA.."bot:reload", 7230, true)
redis:set(GLOBLA.."Bot:TgOwnerBot", config.TgOwner or 0)
...
local hash = "Bot:TgSudos"
local list = redis:smembers(GLOBLA..hash)
local OwnerTG_ = redis:get(GLOBLA.."Bot:TgOwnerBot")
local text = "List of Authorities :\n"
local user_info_ = redis:get(GLOBLA.."user:Name" .. OwnerTG_)
...
redis:del(GLOBLA.."Bot:TgSudos")
redis:del(GLOBLA.."Bot:TgOwnerBot")
...
local TGdata = redis:get(GLOBLA.."Botid" .. bot_id) or "\n"
local BotTGdata = redis:get(GLOBLA.."TgOwnerBot" .. config.TgOwner) or "\n"
if redis:get(GLOBLA.."Rank:Data") then
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. TGdata .. "\027[00m")
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. BotTGdata .. "\027[00m")
for v, user in pairs(config.Sudo_Users) do
local SudoData = redis:get(GLOBLA.."TgSudos" .. user)
if SudoData then
...
local lists = redis:smembers(GLOBLA.."Bot:TgSudos")
for k, v in pairs(lists) do
setnumbergp_three(v)
end
redis:setex(GLOBLA.."bot:reload", 7230, true)
...
local Tg_Owner = redis:get(GLOBLA.."Bot:TgOwnerBot")
...
-----------------------function is_TG----------by Globla------------------------
local is_TG = function(msg)
local var = false
if msg.sender_user_id_ == tonumber(Tg_Owner) then
var = true
end
return var
end
local is_TgiD = function(user_id)
local var = false
if user_id == tonumber(Tg_Owner) then
var = true
end
return var
end
...
local var = false
if redis:sismember(GLOBLA.."Bot:TgSudos", msg.sender_user_id_) then
var = true
end
if msg.sender_user_id_ == tonumber(Tg_Owner) then
var = true
...
local var = false
if redis:sismember(GLOBLA.."Bot:TgSudos", user_id) then
var = true
end
if user_id == tonumber(Tg_Owner) then
var = true
...
------------------function Join CH------------------------------------------
function Tg_JoinCh(msg)
local var = true
if redis:get(GLOBLA.."Tgjoin1") then
local channel = ''..redis:get(GLOBLA..'Tgch1')..''
...
if is_TgiD(msg.sender_user_id_) then
Globla  = "المطور الاساسي 👨‍🔧"
...
local user_id
local user = false
local list1 = redis:smembers(GLOBLA.."Bot:TgSudos")
...
if user_info_ then 
local sudoed = tonumber(Tg_Owner)
local iD_Globla = [[
...
redis:incr(GLOBLA..'tgaddcon'..msg.chat_id_..':'..msg.sender_user_id_)
...
if run == "False" or bot_id == 0 or Tg_Owner == 0 then
...
redis:set(GLOBLA.."TgSudos" .. user, "> Sudo User ID : " .. sudouserid .. [[
...
redis:set(GLOBLA.."TgOwnerBot" .. Tg_Owner, "> Bot Owner ID : " .. botowneruserid .. [[
...
getUser(Tg_Owner, outputbotowner)
--------------------function outputbot----------------------------------------
...
-----------ch join------------------------------------------------------
if redis:get(GLOBLA..'Tgch'..msg.sender_user_id_) then
redis:del(GLOBLA..'Tgch'..msg.sender_user_id_)
...
return false 
elseif data.description == "Bad Request: chat not found" then -- التحقق من المعرف (@tgchh)
...
if is_owner(msg.sender_user_id_, msg.chat_id_) and not Tg_JoinCh(msg) or is_monshi(msg.sender_user_id_, msg.chat_id_) and not Tg_JoinCh(msg) or is_sudo(msg) and not Tg_JoinCh(msg) then
return false
end
-------------------------------leave groups----------------------------------------------------------------------
...
if text:match('^رفع مميز عام$') and is_TG(msg) and msg.reply_to_message_id_ ~= 0  then
...
---------------------------ADD vipmems BY USER--------------------------------------------------------------------
if text:match('^رفع مميز عام @(%S+)$') and is_TG(msg) then
...
---------------------------ADD vipmems BY ID--------------------------------------------------------------------
if text:match('^رفع مميز عام (%d+)$') and is_TG(msg) then
...
if text:match('^تنزيل مميز عام$') and is_TG(msg) and msg.reply_to_message_id_ ~= 0 then
...
------------------------DEL vipmems BY USER-----------------------------------------------------------------------
if text:match('^تنزيل مميز عام @(%S+)$') and is_TG(msg) then
...
--------------------------DEL vipmems BY ID---------------------------------------------------------------------
if text:match('^تنزيل مميز عام (%d+)$') and is_TG(msg) then
...
--------------------------gban_by_reply---------------------------------------------------------------------
if text:match('^حظر عام$') and is_TG(msg) and msg.reply_to_message_id_ then
function ban_by_reply(extra, result, success)
local hash = 'bot:gban:'
if is_momod(result.sender_user_id_, result.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '☑┇ عذراً لا استطيع (حظر،طرد،كتم)المدراء والادمنيه ❗️', 1, 'md')
...
--------------------------ban_by_username---------------------------------------------------------------------
if text:match('^حظر عام @(%S+)$') and is_TG(msg) then
...
------------------------ban_by_id-----------------------------------------------------------------------
if text:match('^حظر عام (%d+)$') and is_TG(msg) then
...
-----------------------ungban_by_reply------------------------------------------------------------------------
if text:match('^الغاء العام$') and is_TG(msg) and msg.reply_to_message_id_ then
function ban_by_reply(extra, result, success)
local hash = 'bot:gban:'
local user_info_ = redis:get(GLOBLA..'user:Name' .. result.sender_user_id_)
local UserGlobla = user_info_
...
--------------------------ban_by_username---------------------------------------------------------------------
if text:match('^الغاء العام @(%S+)$') and is_TG(msg) then
...
------------------------ban_by_id-----------------------------------------------------------------------
if text:match('^الغاء العام (%d+)$') and is_TG(msg) then
...
-------------ADD GLOBLA_SUDO----------------------------------------------------------------------------------
if is_TG(msg) and text:match('^رفع مطور$') and msg.reply_to_message_id_ then
...
-------------ADD GLOBLA_SUDO----------------------------------------------------------------------------------
if text:match('^رفع مطور @(%S+)$') and is_TG(msg) then
...
-------------ADD GLOBLA_SUDO----------------------------------------------------------------------------------
if text:match('^رفع مطور (%d+)$') and is_TG(msg) then
...
--------------REM GLOBLA_SUDO---------------------------------------------------------------------------------
if is_TG(msg) and text:match('^تنزيل مطور$') and msg.reply_to_message_id_ then
...
--------------REM GLOBLA_SUDO---------------------------------------------------------------------------------
if text:match('^تنزيل مطور @(%S+)$') and is_TG(msg) then
...
--------------REM GLOBLA_SUDO---------------------------------------------------------------------------------
if text:match('^تنزيل مطور (%d+)$') and is_TG(msg) then
...
if not is_TG(msg) then
send(msg.chat_id_, msg.id_, 1, '👨‍🔧┇ للمطور الاساسي فقــــــــط', 1, 'md')
else
redis:setex(GLOBLA..'Tgch'..msg.sender_user_id_,300,true)
...
if not redis:get(GLOBLA..'Tgch1') then
send(msg.chat_id_, msg.id_, 1, '™️┇ لم يتم وضع القناة\n🔘┇ لتعين  القناة ارسل .....\n☑┇ `تعين قناة الاشتراك `\n➖', 1, 'md')
return false 
end
if redis:get(GLOBLA..'Tgch1') then
send(msg.chat_id_, msg.id_, 1, "👤┇ بواسطه » "..tmGlobla(msg).."\n🚫┇ تم تفعيل الاشتراك الاجباري\n ✓ ", 1, 'md')
redis:set(GLOBLA.."Tgjoin1", true)
...
local Tgch2 = redis:get(GLOBLA.."Tgch1")
if Tgch2 then
send(msg.chat_id_, msg.id_, 1, '📡┇ *قناة الاشتراك* : ['..Tgch2..']', 1, 'md')
else
...
local Tgcontact = (tonumber(redis:get(GLOBLA.."tgaddcon"..msg.chat_id_..":"..result.id_) or 0))
if result.id_ then
if tonumber(result.id_) == tonumber(Tg_Owner) then
t = "مطور اساسي 👨‍🔧"
...
local msgs = tonumber(redis:get(GLOBLA.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local Tgcontact = (tonumber(redis:get(GLOBLA.."tgaddcon"..msg.chat_id_..":"..msg.sender_user_id_) or 0))
...
local msgs = tonumber(redis:get(GLOBLA.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "⏱┇ تفاعلك : ( *"..TG_TM_NM(msgs).."* ) 🤹‍♂️", 1, 'md')
end
end
--------------------Tg concat add----------------------------------------
if text == 'جهاتي' then
send(msg.chat_id_, msg.id_, 1, '📞┇ عدد جهاتك ← * '..(tonumber(redis:get(GLOBLA..'tgaddcon'..msg.chat_id_..':'..msg.sender_user_id_) or 0))..' *',1,'md')
end
--------------------ID BY REPLY------------------------------------------
if text:match("^ايدي$") and msg.reply_to_message_id_ ~= 0 then
...
local Tgcontact = (tonumber(redis:get(GLOBLA.."tgaddcon"..msg.chat_id_..":"..result.sender_user_id_) or 0))
if result.id_ then
if tonumber(result.sender_user_id_) == tonumber(Tg_Owner) then
...
local loctgt = {string.match(text, "^([Aa]ction) (.*)$")}
if loctgt[2] == "text" then
sendaction(msg.chat_id_, "Typing")
end
if loctgt[2] == "video" then
sendaction(msg.chat_id_, "RecordVideo")
end
if loctgt[2] == "voice" then
sendaction(msg.chat_id_, "RecordVoice")
end
if loctgt[2] == "photo" then
...
-------------------USERS----------------------------------------------------
if text == 'المشتركين' or text == 'عدد المشتركين' and is_TG(msg) then
...
local botnamess = redis:get(GLOBLA.."Globlanams") or "العالمي"
local tgSudos1 = redis:scard(GLOBLA.."Bot:TgSudos") or "0"
local tgAdmins = redis:scard(GLOBLA.."Bot:Admins") or "0"
local tggban = redis:scard(GLOBLA.."bot:gban:") or "0"
local tgvipmems = redis:scard(GLOBLA.."bot:vipmems:") or "0"
local tggps = redis:scard(GLOBLA.."bot:groups") or "0"
local tgusers = redis:scard(GLOBLA.."bot:userss") or "0"
send(msg.chat_id_, msg.id_, 1, "*- احصائيات البـــــوت ←*\n* -------------------»*\n🤖┇اسم البوت ← *" .. botnamess .. "*\n🔘┇عدد المطورين ← *"..tgSudos1.."*\n🔘┇عدد الادمنيه ← *"..tgAdmins.."*\n🔘┇عدد الكروبات ← * "..tggps.."*\n🔘┇عدد المشتركين خاص ← *" .. tgusers .. "*\n🔘┇عدد المحظورين عام ← *"..tggban.."*\n🔘┇عدد المميزين عام ← * "..tgvipmems.."*\n📡┇قناة السورس ← [Globla](t.me/Globla)\n✓", 1,"md")
end
end
------------------GPS AND USERS-------------------------------------------
...
📮┇تفعيل / تعطيل رفع ادمن
📮┇تفعيل / تعطيل رفع مميز
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
👨‍💻┇*مطور البوت* : []] .. UserGlobla .. [[] ‌‏]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end end end end
...
--------------------------------------------------------------------------------------------------------------
if text == 'وقت تشغيل البوت' and is_TG(msg) then
local cm = io.popen("uptime -p"):read("*all")
local ResultUptimeServer = GetUptimeServer(cm, lang)
if 4 > string.len(ResultUptimeServer) then
if lang == "Ar" then
...
local tgbanned = redis:scard(GLOBLA.."bot:banned:" .. msg.chat_id_) or "0"
redis:del(GLOBLA.."bot:banned:" .. msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المحضورين ← *"..tgbanned.."*\n☑┇تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
end end
----------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المحضورين عام ← *"..tggban.."*\n☑┇تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
end end
----------------------------------------
...
local tgmomod = redis:scard(GLOBLA.."bot:momod:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┇عدد الأدمنيهہ‌‏ ← *"..tgmomod.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
...
local tgowners = redis:scard(GLOBLA.."bot:owners:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المدراء ← *"..tgowners.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
...
local tgmonshis = redis:scard(GLOBLA.."bot:monshis:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المنشئين ← *"..tgmonshis.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
...
local tgSudos1 = redis:scard(GLOBLA.."Bot:TgSudos") or "0"
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المطورين ← *"..tgSudos1.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
local hash = "Bot:TgSudos"
local list = redis:smembers(GLOBLA..hash)
for k, v in pairs(list) do
local t = tonumber(v)
...
local tgAdmins = redis:scard(GLOBLA.."Bot:Admins") or "0"
send(msg.chat_id_, msg.id_, 1, "📉┇عدد ادمنيه البوت ← *"..tgAdmins.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
...
local tgvipmem = redis:scard(GLOBLA.."bot:vipmem:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المميزين ← *"..tgvipmem.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
...
local tgkeed = redis:scard(GLOBLA.."bot:keed:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المقيدين ← *"..tgkeed.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
local hash = "bot:keed:" .. msg.chat_id_
local list = redis:smembers(GLOBLA..hash)
for k, v in pairs(list) do
...
local tgvipmems = redis:scard(GLOBLA.."bot:vipmems:") or "0"
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المميزين عام ← *"..tgvipmems.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
...
local tgmuted = redis:scard(GLOBLA.."bot:muted:" .. msg.chat_id_) or "0"
redis:del(GLOBLA.."bot:muted:" .. msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, "📉┇عدد المكتومين ← *"..tgmuted.."*\n☑┇ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
end end end
-------------------kickedlist--------------------------------------------
...
local tgmonshis = redis:scard(GLOBLA.."bot:monshis:" .. chattid) or "0"
local tgbanned = redis:scard(GLOBLA.."bot:banned:" .. chattid) or "0"
local tgowners = redis:scard(GLOBLA.."bot:owners:" .. chattid) or "0"
local tgmuted = redis:scard(GLOBLA.."bot:muted:" .. chattid) or "0"
local tgkeed = redis:scard(GLOBLA.."bot:keed:" .. chattid) or "0"
local tgmomod = redis:scard(GLOBLA.."bot:momod:" .. chattid) or "0"
local tgvipmem = redis:scard(GLOBLA.."bot:vipmem:" .. chattid) or "0"
...
send(msg.chat_id_, msg.id_, 1, "👮‍♂️┇ المدير » ["..owner.."]\n🔘┇ ["..title_name(chattid).."]("..(link or "t.me/Globla")..")\n🔘┇ عدد المــــدراء   » *"..tgowners.."*\n🔘┇ عدد المنشئين   » *"..tgmonshis.."*\n🔘┇ عدد الادمنيـــه   » *"..tgmomod.."*\n🔘┇ عدد المكتومين  » *"..tgmuted.."*\n🔘┇ عدد المحظورين » *"..tgbanned.."*\n🔘┇ عدد المقيديــن   » *"..tgkeed.."*\n🔘┇ عدد المميزيـــن  » *"..tgvipmem.."*\n🔘┇ `"..chattid.."`\n👨‍💻┇ المطور » ["..sudo.."]\n✓", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
local Tgkick = redis:get(GLOBLA.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
if Tgkick == "kick_mee" then
...
local Tgunkick = redis:get(GLOBLA.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
if Tgunkick == "kick_no" then
redis:del(GLOBLA.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:del(GLOBLA.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
...
local tgtext = (tonumber(redis:get(GLOBLA.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local tgsticker = (tonumber(redis:get(GLOBLA.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local ptGhoto = (tonumber(redis:get(GLOBLA.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local tgVoice = (tonumber(redis:get(GLOBLA.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local tgGif = (tonumber(redis:get(GLOBLA.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local tgVideo = (tonumber(redis:get(GLOBLA.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local tgSelfVideo = (tonumber(redis:get(GLOBLA.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local tgcon = (tonumber(redis:get(GLOBLA.."tgaddcon"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
if result.first_name_ then
if #result.first_name_ < 25 then
else
for yybyy in string.gmatch(result.first_name_, "[^%s]+") do
...
send(msg.chat_id_, msg.id_, 1, tgmsgsss, 1, 'md')
end
getUser(msg.sender_user_id_, get_me)
end end
-------------------------------------
...
redis:set(GLOBLA.."Bot:TgBotAccount", data.value_.value_)
end end end end  end
