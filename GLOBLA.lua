...
local group = redis:smembers(GLOBLA..'bot:gpsby:id')   
local w = 0
local q = 0
for i = 1, #group do
...
redis:srem(GLOBLA.."bot:gpsby:id",group[i])  
rem_group(group[i])   
...
local pv = redis:smembers(GLOBLA..'usersbot')   
local sendok = 0
for i = 1, #pv do
...
redis:srem(GLOBLA.."usersbot",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
os.execute('cd plugins_;wget https://raw.githubusercontent.com/DevGlobla/GLOBLA/master/plugins_/help_rep.lua') 
send(msg.chat_id_, msg.id_, 1, "👤┇ بواسطه » "..tmGlobla(msg).."\n☑┇ تم تحديث السورس \n ✓ ", 1, 'md')
dofile('GLOBLA.lua')  
...
if not is_KP(msg) then  
send(msg.chat_id_, msg.id_, 1,'*📮¦ هاذا الامر خاص بالمطور الاساسي *\n', 1, 'md') 
return false
end
local Get_Files, res = https.request("https://raw.githubusercontent.com/DevGLOBLA/DevGLOBLA/master/getfile.json")
if res == 200 then
...
if text and text:match('تعطيل ملف (.*)') and is_KP(msg) then  
...
local json_file, res = https.request("https://raw.githubusercontent.com/DevGLOBLA/DevGLOBLA/master/plugins_/"..file)
if res == 200 then
...
if text and text:match('تفعيل ملف (.*)') and is_KP(msg) then  
